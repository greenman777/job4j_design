...
class SimpleMapTest {
    ...
    @Test
    void whenCheckPutAndRemove() {
        assertThat(map.put(7, "100")).isTrue();
        assertThat(map).hasSize(5);
        assertThat(map.put(null, "1000")).isTrue();
        assertThat(map).hasSize(6);
        assertThat(map.remove(7)).isTrue();
        assertThat(map).hasSize(5);
        assertThat(map.remove(null)).isTrue();
        assertThat(map).hasSize(4).contains(1, 2, 3, 4);
    }
    ...
    void whenCheckPutAndGet() {
        assertThat(map.put(5, "200")).isTrue();
        assertThat(map).hasSize(5);
        assertThat(map.get(null)).isNull();
        assertThat(map.get(7)).isNull();
        assertThat(map.put(null, "1000")).isTrue();
        assertThat(map.get(null)).isEqualTo("1000");
        assertThat(map.get(5)).isEqualTo("200");
        assertThat(map).hasSize(6).contains(1, 2, 3, 4, 5, null);
    }

    @Test
    void whenCheckPutAndIteratorConcurrentModification() {
        Iterator<Integer> it = map.iterator();
        assertThat(map.put(7, "700")).isTrue();
        assertThatThrownBy(it::next).isInstanceOf(ConcurrentModificationException.class);
    }

    @Test
    void whenCheckIteratorNoSuchElementException() {
    ...
    @Test
    void whenCheckPutBusy() {
        assertThat(map.put(100, "100")).isFalse();
    }
}